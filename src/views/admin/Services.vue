<template>
  <div class="servicos-admin">
    <div class="header">
      <h3>🧰 Gerenciar Serviços de Manutenção</h3>
    ...
    <!-- Tabela de serviços -->
    <div v-if="servicosFiltrados.length" class="table-responsive mt-4">
    ...
    <p v-else class="text-center mt-4 text-muted">Nenhum serviço encontrado.</p>
...
onMounted(() => {
  const armazenados = localStorage.getItem('servicos')
  if (armazenados) services.value = JSON.parse(armazenados)
})
...
function editarServico(index) {
  form.value = { ...services.value[index] }
...
function salvarServico() {
  ...
  if (editando.value !== null) {
    services.value[editando.value] = { ...form.value }
  } else {
...
function salvarLocal() {
  localStorage.setItem('servicos', JSON.stringify(services.value))
}
...
const servicosFiltrados = computed(() => {
  if (!busca.value) return services.value
